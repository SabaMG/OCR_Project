...
void feed_forward_pass2(Layer network[], double inputs[]) {
	...
	for (size_t j = 0; j < network[1].size; j++) {
		// Put the bias of the precedent layer neuron in the value of the current hidden neuron
		//network[1].neurons[j].value = network[0].neurons[j].bias;
		double activation = network[1].neurons[j].bias;

		for (size_t k = 0; k < network[0].size; k++)
			//network[1].neurons[j].value += network[0].neurons[k].weights[j] * network[0].neurons[k].value;
			activation += network[0].neurons[k].value * network[1].neurons[j].weights[k];
		
		//network[1].neurons[j].value = sigmoid(network[1].neurons[j].value);
		network[1].neurons[j].value = sigmoid(activation);
	}
	...
	for (size_t j = 0; j < network[2].size; j++) {
		//network[2].neurons[j].value = network[1].neurons[j].bias;
		double activation = network[2].neurons[j].bias;
		for (size_t k = 0; k < network[1].size; k++)
			//network[2].neurons[j].value += network[1].neurons[k].weights[j] * network[1].neurons[k].value;
			activation += network[1].neurons[k].value * network[2].neurons[j].weights[k];
		network[2].neurons[j].value = sigmoid(activation);
	}

	//softmax(network[2].size, network[2].neurons);
}

void backpropagation_pass2(Layer network[], double expected_output[], double learning_rate) {

	for (size_t i = 0; i < network[2].size; i++) {
		//double n_value = network[2].neurons[i].value;
		//network[1].neurons[i].delta = n_value * (1 - n_value) * (n_value - expected_output[i]);
		double error = expected_output[i] - network[2].neurons[i].value;
		network[2].neurons[i].delta = error * dSigmoid(network[2].neurons[i].value);
	}

	for (size_t j = 0; j < network[1].size; j++) {
		double error = 0.0f;
		for (size_t k = 0; k < network[2].size; k++)
			error += network[2].neurons[k].delta * network[2].neurons[k].weights[j];
		network[1].neurons[j].delta = error * dSigmoid(network[1].neurons[j].value);
	}

	for (size_t j = 0; j < network[2].size; j++) {
		network[2].neurons[j].bias += network[2].neurons[j].delta * learning_rate;
		for (size_t k = 0; k < network[1].size; k++)
			network[2].neurons[j].weights[k] += network[1].neurons[k].value * network[2].neurons[j].delta * learning_rate;
	}

	for (size_t j = 0; j < network[1].size; j++) {
		network[1].neurons[j].bias += network[1].neurons[j].delta * learning_rate;
		for (size_t k = 0; k < network[0].size; k++)
			network[1].neurons[j].weights[k] += network[0].neurons[k].value * network[1].neurons[j].delta * learning_rate;
	}
}
